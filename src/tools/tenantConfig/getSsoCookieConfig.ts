...
export const getSsoCookieConfigTool = {
  name: 'getSsoCookieConfig',
  title: 'Get SSO Cookie Config',
  description: 'Retrieve the SSO cookie configuration for the tenant',
  scopes: SCOPES,
  annotations: {
    readOnlyHint: true,
    openWorldHint: true
  ...
  async toolFunction(_args: Record<string, never>) {
    ...
    try {
      ...
      return createToolResponse(`Failed to get SSO cookie config: ${error.message}`);
    }
  }
};
